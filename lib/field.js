...
function Field(property, label) {
  var stack = []
    , isArray = false
    , minArraySize = 0
    , maxArraySize = Number.MAX_VALUE
    , arrayMessage = ""
    , fieldLabel = label || property;

  this.name = property;
  this.__required = false;
  this.__validateCustomIfEmpty = false;

  this.ignoreErrors = function() {
    return !this.__required && !this.__validateCustomIfEmpty;
  }
  ...
  this.array = function(min, max, message) {
    isArray = true;
  ...
  this.run = function (source, form, options, cb) {
    ...
    function runStack(foo, cb) {
      // error for nested arrays with validator.js methods, we can't handle them reliably
      if (options.throwNestedArrayError && Array.isArray(foo)) {
        errors.push('express-form cannot process nested arrays of form parameters');
        return cb(null);
      }
    ...
    if (isArray) {
      ...
      if (value.length < minArraySize) errors.push(arrayMessage.replace("%s", fieldLabel) || fieldLabel + " has " + value.length + " elements. It should have at least " + minArraySize);
      if (value.length > maxArraySize) errors.push(arrayMessage.replace("%s", fieldLabel) || fieldLabel + " has " + value.length + " elements. It should have " + maxArraySize + " or fewer");
...
Field.prototype.array = function(min, max, message) {
  return this.array(min, max, message);
};
...
Field.prototype.ifNullOrNaN = function (replacement) {
    return this.add(function (value) {
        if (object.isUndefined(value) || null === value || isNaN(value) || '' === value) {
            return replacement;
        }
        return value;
    });
...
Field.prototype.required = function (messageOrValue, message) {
  var placeholderValue;
  if (arguments.length == 1) {
    message = messageOrValue;
    placeholderValue = "";
  } else {
  	placeholderValue = messageOrValue;
  }
  this.__required = true;
  return this.add(function(value) {
...
Field.prototype.validateCustomIfEmpty = function () {
  this.__validateCustomIfEmpty = true;
