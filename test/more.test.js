...
module.exports = {
  ...
  "field : arrays": function () {
    ...
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array(5, 10))(request, {});
    assert.equal(request.form.errors[0], "field has 4 elements. It should have at least 5");
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array(1, 3))(request, {});
    assert.equal(request.form.errors[0], "field has 4 elements. It should have 3 or fewer");
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array(1, 3, "Incorrect number of camelids"))(request, {});
    assert.equal(request.form.errors[0], "Incorrect number of camelids");
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array(1, 3, "Incorrect number of camelids in %s"))(request, {});
    assert.equal(request.form.errors[0], "Incorrect number of camelids in field");
    // Cope with broken arguments.
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array("fish", 3))(request, {});
    assert.strictEqual(request.form.errors.length, 0);
    var request = { body: { field: ["Alpacas?", "Llamas!!?", "Vicunas!", "Guanacos!!!"] } };
    form(field("field").array(1))(request, {});
    assert.strictEqual(request.form.errors.length, 0);
